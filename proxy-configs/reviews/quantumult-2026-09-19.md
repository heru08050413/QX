...
## 2026-09-20 勘误：v5.2.1 导入兼容性回退

用户反馈原生导入在第 87 行报语法错误，正对应上轮改成 HTTPS 的 `server_check_url`。本轮仅将该生效行恢复为 `server_check_url=http://cp.cloudflare.com/generate_204`，其余生效配置不变；文件头修订号更新为 v5.2.1，原行号保留。

上轮将“桌面 HTTPS HEAD 返回 204”当作采用此写法的依据并不充分：它不证明手机 QX 的解析器接受该参数。撤回下表中“升级 HTTPS”的当前有效建议；下方 v5.2.0 内容仅作为历史记录。官方样例采用 HTTP 测速地址，但这也不构成所有 QX 版本都不支持 HTTPS 的证明。

测试已纠正：从发布基线 `e8fa202efd94195ed0e095fc2ebf1564b33a2099` 断言只能改变这一条生效行；把回归成 HTTPS 设为负向用例；联网检查直接读取配置中的 HTTP 地址，禁止跟随跳转。节点 TLS 和 MITM 的证书校验没有降低，规则、订阅、DNS、去广告及其他客户端均未变更。HTTP 探针不验证目标站点 TLS，也可能受链路劫持影响，不能作为真实业务/吞吐验收。

手机验收：刷新或重新导入原 `quantumult_B_V26.00` 链接，核对 v5.2.1 和第 87 行为 HTTP，再确认能否完整导入。尚无 QX 原生测试环境，不能在用户复测前宣称整个配置已通过手机验收。机场占位符与 `geoip` 附加参数等既有待核实项不在本次单行回退范围内。

本轮实测通过：固定基线单行差异检查、48 个美国节点名称样本、12 项负向变异；32 个启用资源与 8 个重写脚本依赖可下载、13 份 JavaScript 语法通过，配置中的 HTTP HEAD 探针无跳转直接返回 204。未执行第三方脚本、未访问真实机场订阅。
