...
- `Stash_Script_V26.00.stoverride` + `scripts/zhihu.js` + `scripts/xiaohongshu.js` — **信息流脚本级去广告**(知乎/小红书)。开屏/独立广告接口靠 url-rewrite 整条 reject 即可(见上一份);**信息流里混排的广告**要解析 JSON、逐条删广告保正文,必须用脚本。
  > 架构关键(避免重蹈"Scripts Not Downloaded"):脚本 JS **镜像进本仓库 `scripts/`**、`.stoverride` 的 `script-providers.url` 只引**自己仓库的 raw 链接**——第三方死链风险归零,只剩"App 改接口→标记字段变"的逻辑腐坏(用得多的 App 社区盯得紧,同步上游即可)。上游源 fmz200/wool_scripts(作者 @fmz200),已把上游两个 URL 加入巡检 EXTRA_URLS,上游死/大改即回本仓库 diff 同步。**B站脚本(kokoryh/Sparkle)依赖 `engine=webview` + protobuf 二进制解析,Stash 脚本引擎无 webview、转换器也不吐 `engine` 字段 → 无法忠实移植,不做**。
