...
新配置，内部修订 `1.0.1`，2026-09-24。原有 Stash、Shadowrocket、Egern、Loon、Quantumult X 文件均不改动。
...
3. 更新机场配置，查看 `手动节点` 和 `稳定切换` 是否列出真实节点，并确认至少两条测速成功；核对节点证书验证未被机场关闭。才开启 VPN。**空节点集可能退化为 DIRECT，本覆写不是空订阅下的防泄漏开关。**没有有效节点就不要启动。
4. 新策略的 `默认代理` 与 `YouTube` 初始选项是 **REJECT**。确认节点后手动改为 `默认代理 → 稳定切换`、`YouTube → 稳定切换`。在 `AI` 中测试目标服务；需要固定地区时直接选已验证支持服务的节点。
